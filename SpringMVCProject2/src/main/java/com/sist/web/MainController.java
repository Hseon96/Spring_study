 ...
 *  			기타 : 사용자가 보내준 데이터
 *  				 ---------------- 2. VO 단위 (커맨드객체) 
 *  				----------------- 3. 낱개 (각 데이터형으로 받을지 여부 확인) 
 *  				*** 순서는 없다 
 *  				*** request를 사용하지 않는 경우 : Model (전송객체 이용)
 *  	리턴형: String, void
 *  		  ------ 90% (값을 받아서 출력하는 JSP를 설정) : forward
 *  		  ------ 재전송 : redirect:~ : sendRedirect
 *  		  void : ajax, 다운로드 , axios(Vue, React) ...
 */
@Controller // =Model class // = 메모리 할당을 해야 찾아준다// 꼭 반드시 @올려놓을 것!!! 
public class MainController {
	@GetMapping("main/input.do")
	public String main_input() // 매개변수 없음 
	{
		return "main/input";  // 이렇게 두면 못찾음 .. => application-context가서 사용자 정의 등록하고 메모리 등록하고 와야한다
	}
	
	// no=1&name=홍길동&avg=10.5&isadmin=true  ==> String으로 다 쓸순있지만 변환해서 쓰는게 더 편하다!! 나중에 다 변환해야하니깐   // 예외 : 페이징처리
