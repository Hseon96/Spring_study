...
// 스프링에서 클래스 관리 제외 (~VO(사용자 정의 데이터형--싱글턴), Mapper(interface))
...
public class BoardDAO {
	...
	@Insert("INSERT INTO spring_board VALUES("
			+"#{no},#{subject},#{name},#{content},#{pwd},SYSDATE,0)")*/
	public void boardInsert(BoardVO vo)
	{
		mapper.boardInsert(vo); // 리턴값이 없기 때문에!
	}
	 ...
	 * @Select("SELECT no,name,subject,content,TO_CHAR(regdate,'YYYY-MM-DD') as dbday,hit "
			...
			+"WHERE no=#{no}")
	 */
	public BoardVO boardUpdateData(int no) // 위에 detail 재사용
	{
		return mapper.boardDetailData(no);
	}
	/*
