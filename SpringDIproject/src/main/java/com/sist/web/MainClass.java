...
public class MainClass {
	public static void main(String[] args) {
		 ...
		 * 		BeanFactory : Core(DI)
		 * 			|
		 * 		Application	: Core+AOP			
		 * 			|										|
		 *   WebApplicationContext          AnnotationConfigApplicationContext(5버전)
		 *   	Core+AOP+MVC					Core+AOP+Annotation
		 */
		ApplicationContext app=new ClassPathXmlApplicationContext("app.xml"); // 파싱
		SawonVO sa1=(SawonVO)app.getBean("sa1"); // getBean안에 들어가있는게 ID = 등록된 ID를 가지고 해당 객체를 찾아온다 => DL
		sa1.print();
		
		SawonVO sa2=app.getBean("sa2",SawonVO.class); // 형변환 대신 하는 방법
		sa2.print();
		
		SawonVO sa3=(SawonVO)app.getBean("sa3");
		sa3.print();
		// 메모리 할당 => HashMap에 저장 (객체의 주소가 동일 =>  싱글턴(하나의 메모리만 사용))
		// DI(값 주입 ==> database,서버)
	}
}
